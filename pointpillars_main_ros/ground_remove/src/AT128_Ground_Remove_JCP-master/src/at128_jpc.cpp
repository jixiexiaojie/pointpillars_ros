...
void ProjectionJpc::ptsPub() {
  for (int row = 0; row < scan_num; ++row) {
    for (int col = 0; col < horizon_pts_num; ++col) {
      ...
      if (range_image.at<cv::Vec3b>(row, col) == cv::Vec3b(0, 0, 255)) {
        if((pts_full->points[index]).z  <  3)
        if((pts_full->points[index].y>-20)&&(pts_full->points[index].y<10))
        {
  ...
  ros_pts_ground.header.frame_id = "PandarSwift";
  ...
  ros_pts_obstacle.header.frame_id = "PandarSwift";
