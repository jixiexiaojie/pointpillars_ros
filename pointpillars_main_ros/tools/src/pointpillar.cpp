...
TRT::TRT(std::string modelFile, cudaStream_t stream):stream_(stream)
{
  ...
  if (!trtCache.is_open())
  {
    ...
    {
      std::cerr << ": failed to parse onnx model file, please check the onnx version and trt support op!" << std::endl;
      exit(-1);
    }
    ...
    // networkConfig->setFlag(nvinfer1::BuilderFlag::kINT8);
...
int PointPillar::doinfer(void*points_data, unsigned int points_size, std::vector<Bndbox> &nms_pred)
{
  ...
  pre_->generateVoxels((float*)points_data, points_size, params_input_, voxel_features_, voxel_num_, voxel_idxs_);
  ...
  pre_->generateFeatures(voxel_features_, voxel_num_, voxel_idxs_, params_input_, features_input_);
